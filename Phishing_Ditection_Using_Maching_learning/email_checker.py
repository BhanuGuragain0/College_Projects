    ...
    def predict(self, text):
        ...
        try:
            ...
            if np.max(probs) < 0.8:  # Confidence threshold
                self.active_learning_queue.append({"text": text, "confidence": float(np.max(probs))})
    ...
    def optimize_hyperparameters(self, dataset, n_trials=20):
        ...
        def objective(trial):
            ...
            try:
                ...
                metrics = self._compute_metrics(self.model, dataset["test"])
...
async def get_model_version():
    ...
    if not detector:
        raise HTTPException(status_code=503, detail="Model not initialized")
    return {"version": detector.version, "device": str(detector.device)}
...
async def retrain():
    ...
    try:
        ...
        return {"status": "Retraining initiated", "uncertain_samples": queue_size}
    except Exception as e:
